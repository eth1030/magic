...
class ExecutionMagics(Magics):
    ...
    def praxi(self,line='', cell=None, local_ns=None):
        ...
        if cell:
            # parser = argparse.ArgumentParser(description='Arguments for Praxi line magic.')
            # parser.add_argument('-c','--command', help = 'Shell command',required=True)
            # parser.add_argument('-l', '--label', help='Application label', required=True)
            # # parser.add_argument('-c', '--command', help='Shell command', required=True)
            # args = vars(parser.parse_args(cell))
            # command = args['command']
            # print(command)
            # label = args['label']
            # print(label)
            # expr = self.shell.transform_cell(command)
        ...
        # seeing multi classifier example
        # p3 = subprocess.Popen(['python3', os.path.join(dirname, 'main.py'),'-t',os.path.join(dirname, 'demo_tagsets/sl_test_tag'),
        #     '-s',os.path.join(dirname, 'tagsets'),'-o',os.path.join(dirname, 'results'), '-i', os.path.join(dirname, 'demomodel.vw'), '-l'], 
